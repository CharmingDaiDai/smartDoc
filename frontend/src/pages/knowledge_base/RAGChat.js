...
const RAGChat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log("Using RAG method:", ragMethodId, "with params:", ragParams);
  ...
  const handleStreamResponse = async () => {
    ...
    try {
      // 构建API URL和参数
      const { apiUrl, queryParams } = buildApiRequestParams(userQuestion, knowledgeBase, ragMethodType, ragParams);
        ...
        try {
          ...
          if (result.success) {
            ...
            if (result.docs && Array.isArray(result.docs) && result.docs.length > 0) {
              ...
              setMessages((prevMessages) =>
                prevMessages.map((msg) =>
                  msg.id === aiMessageId ? { ...msg, sources: sourcesData } : msg
                )
              );
            }
            ...
            if (result.contentDelta) {
              ...
              setMessages((prevMessages) =>
                prevMessages.map((msg) =>
                  msg.id === aiMessageId ? { ...msg, content: contentBuffer } : msg
                )
              );
            }
  ...
  const parseStreamData = (rawData) => {
    ...
    try {
      ...
      // 提取直接内容格式
      if (data.content !== undefined && data.content !== null && data.content !== "[DONE]") {
      ...
      if (data === "[DONE]" || 
          data.content === "[DONE]" || 
          data.text === "[DONE]" ||
          (data.choices && data.choices.length > 0 && data.choices[0].finish_reason === "stop") ||
          data.finished ||
          data.done === true) {
  ...
  const processDocSources = (docs) => {
    ...
    }).filter(source => source.content && source.content !== "未提供内容");  // 过滤空内容
  };
  ...
  const tryRecoverFromError = (event, messageId, contentBuffer) => {
    if (event && event.data && typeof event.data === 'string') {
      ...
      // 只添加看起来像文本内容的部分
      if (textContent && !textContent.includes('"id":') && 
          !textContent.includes('"object":')) {

        setMessages((prevMessages) =>
          prevMessages.map((msg) =>
            msg.id === messageId ? { ...msg, content: contentBuffer + textContent } : msg
          )
        );
      }
    }
  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      <List.Item key={index} style={{ padding: "8px 0" }} className="message-item">
        <div
          style={{
            display: "flex",
            ...
            {isUser ? (
              ...
              <div>
                ...
                {DEBUG_MODE && msg.sources && (
                  <div style={{margin: '8px 0', padding: '4px', background: '#fffbe6', fontSize: '12px', borderLeft: '2px solid #faad14'}}>
                    <div>出处数量: {msg.sources.length}</div>
                    <div>原始数据: {JSON.stringify(msg.sources).substring(0, 100)}...</div>
                  </div>
                )}
  ...
  const SourceItem = ({ source }) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ margin: "6px 0 0 0", fontSize: "13px", color: "#333", lineHeight: "1.6" }}>
          {needsExpand && !expanded ? (
            <>
              <div>{`${source.content.substring(0, 80)}...`}</div>
              <Button 
                type="link" 
                size="small" 
