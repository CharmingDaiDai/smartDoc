  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
      <Avatar 
        ...
        icon={icon}
      />
      <Text strong style={{ color: '#fff', fontSize: '16px' }}>{title}</Text>
    </div>
    <Statistic 
      value={value} 
      suffix={suffix}
...
const ActivityItem = ({ activity }) => {
  ...
  const getActivityIcon = () => {
    switch(activity.type) {
      case 'SUMMARY': return <EyeOutlined style={{ ...iconStyle, color: '#1890ff' }} />;
      case 'KEYWORDS': return <HighlightOutlined style={{ ...iconStyle, color: '#52c41a' }} />;
      case 'SECURITY': return <SafetyOutlined style={{ ...iconStyle, color: '#faad14' }} />;
      case 'POLISH': return <EditOutlined style={{ ...iconStyle, color: '#722ed1' }} />;
      case 'UPLOAD': return <FileProtectOutlined style={{ ...iconStyle, color: '#1890ff' }} />;
      case 'DOWNLOAD': return <FileTextOutlined style={{ ...iconStyle, color: '#1890ff' }} />;
      default: return <FileTextOutlined style={{ ...iconStyle, color: '#1890ff' }} />;
    }
  };
    ...
    >
      <List.Item.Meta
        avatar={
          <Avatar 
            style={{ 
              backgroundColor: activity.color === 'blue' ? '#e6f7ff' : 
                              activity.color === 'green' ? '#f6ffed' :
                              activity.color === 'orange' ? '#fff7e6' :
                              activity.color === 'purple' ? '#f9f0ff' :
                              activity.color === 'cyan' ? '#e6fffb' : '#e6f7ff',
              color: activity.color === 'blue' ? '#1890ff' : 
                    activity.color === 'green' ? '#52c41a' :
                    activity.color === 'orange' ? '#faad14' :
                    activity.color === 'purple' ? '#722ed1' :
                    activity.color === 'cyan' ? '#13c2c2' : '#1890ff',
            }}
        ...
        title={
          <div style={{ fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Tag color={activity.color} style={{ marginLeft: 8, borderRadius: '4px' }}>
              {activity.tag}
            </Tag>
          </div>
        }
...
const Dashboard = () => {
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <Paragraph style={{ marginTop: 16, color: '#8c8c8c' }}>正在加载您的智能文档仪表盘...</Paragraph>
      </div>
    );
  }
  ...
  // 计算使用进度 (模拟数据)
  const usageProgress = Math.min(Math.round((statistics.documents / 100) * 100), 100) || 25;
  ...
  return (
    <div>
      <div style={{ 
        ...
        <Title level={2} style={{ color: 'white', margin: 0 }}>欢迎回来, {currentUser?.username}!</Title>
        ...
        <Col xs={24} sm={12} md={12}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div>
                {currentUser?.isVip ? (
                  <Tag color="gold" icon={<CrownOutlined />}>VIP无限制使用</Tag>
                ) : (
                  <Button type="primary" ghost onClick={() => navigate('/vip/membership')}>
                    升级VIP
                  </Button>
                )}
              </div>
      ...
      <Row gutter={[24, 24]} style={{ marginTop: 24 }}>
        ...
        <Col xs={24} lg={8}>
          <Card 
            ...
            extra={<Button type="link" icon={<AppstoreOutlined />}>全部功能</Button>}
            style={{
              borderRadius: '12px',
              boxShadow: '0 4px 12px rgba(0, 0, 0, 0.05)'
            }}
          >
            <List
              size="large"
              dataSource={[
                { title: '文档摘要', link: '/analysis/summary', icon: <EyeOutlined style={{ color: '#1890ff' }} /> },
                { title: '关键词提取', link: '/analysis/keywords', icon: <HighlightOutlined style={{ color: '#52c41a' }} /> },
                { title: '敏感信息检测', link: '/analysis/security', icon: <SafetyOutlined style={{ color: '#faad14' }} /> },
                { title: '文档润色', link: '/analysis/polish', icon: <EditOutlined style={{ color: '#722ed1' }} /> },
              ]}
              renderItem={item => (
                <List.Item 
                  style={{ 
                ...
                >
                  <Space>
                    <Avatar size="small" style={{ backgroundColor: 'rgba(24, 144, 255, 0.1)' }} icon={item.icon} />
          ...
          >
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                <CrownOutlined style={{ color: '#faad14', marginRight: '8px', marginTop: '4px' }} />
                <div>
                  <Text strong>VIP功能更新</Text>
                  <Paragraph style={{ margin: '4px 0 0' }}>
                    VIP用户现可使用批量处理功能，提高工作效率。
                    <Button type="link" size="small" style={{ padding: '0', display: 'block' }}>
                      了解更多
                    </Button>
                  </Paragraph>
                </div>
